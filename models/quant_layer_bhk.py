...
class weight_quantize_fn(nn.Module):
    def __init__(self, w_bit):
        ...
        assert (w_bit <=16 and w_bit > 0) or w_bit == 32
        self.w_bit = w_bit-1
...
def act_quantization(b, grid, power=False):
...
class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False, emul=False):
        ...
        self.bit = 4
        self.power = False  # Mingu Added
        self.weight_quant = weight_quantize_fn(w_bit=self.bit, power=self.power)
    ...
    def forward(self, x):
        ...
        if self.emul:
            return conv2d_emul(x, self.bit, self.act_alpha, self.weight_quant.wgt_alpha, weight_q, self.bias, self.stride, self.padding, self.dilation, self.groups)
        else:
